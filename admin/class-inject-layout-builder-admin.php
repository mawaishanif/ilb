...
class Inject_Layout_Builder_Admin {
	...
	public function load_assets($hook) {
		...
		wp_enqueue_style('layout_builder', ILB_DIR_URL. 'admin/assets/' .'css/layout_builder.css', array(), $this->version);

		// vendor SCRIPTS
		wp_enqueue_script('inert-script', ILB_DIR_URL. 'admin/assets/' .'js/inert.min.js', array('jquery'), $this->version);
		...
		/*=================================================
		=            NEW INJECT LAYOUT BUILDER            =
		=================================================*/
		

		wp_enqueue_script('inject_builder_script', ILB_DIR_URL. 'admin/assets/' .'js/inject_builder.js', array('dnd-modalBox', 'dnd-shortcodes-mCustomScrollbar', 'dnd-cleditor', 'wp-color-picker','jquery-ui-sortable','jquery-ui-resizable','dnd-shortcodes-fancybox'), $this->version,true);

		wp_localize_script('inject_builder_script', 'dnd_from_WP', array(
			'plugins_url' => plugins_url('inject-layout-builder'),
			'ABdevDND_shortcode_names' => $this->shortcodes->shortcodes('registered'),
			'save' => __('Save', 'dnd-shortcodes'),
			'error_to_editor' => __('<b>Content cannot be parsed</b><br>Please use Text tab instead or Revisions option to undo recently made changes.<br><br>Check the syntax:<br>- Use double quotes for attributes<br>- Every shortcode must be closed. e.g. [gallery ids="1,20"] should be [gallery ids="1,20"][/gallery]', 'dnd-shortcodes'),
		...
		/*=====  End of NEW INJECT LAYOUT BUILDER  ======*/
